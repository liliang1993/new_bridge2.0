<template>
	 <div v-show="show" ref="drag-dialog" class="xa-popup">
	  	<div  v-if='isModal' ref='popup-modal' class="layer"></div>
	  	<div ref="popup" class="popup"  @mousedown = 'click'>
	  		<div ref="title" :class="title.className || 'xa-bg-eee'" class="title"   >
	  			<div class="name">{{ title.text || title }}</div>
	  			<div  @click.stop="close()" class="close"></div>
	  		</div>
...
<style scoped>
	...
	.xa-popup .popup>.main{overflow-y:auto;overflow-x:hidden;padding: 10px;}
